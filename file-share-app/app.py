...
<div class="container">
<h1>🚀 FileShare</h1>
<p>Upload any file → Get instant link</p>
...
fileInput.onchange = () => {
    if(fileInput.files[0]) {
        ...
        uploadBtn.innerHTML = '🚀 Upload ' + fileInput.files[0].name;
    }
};
async function uploadFile() {
    ...
    if(data.success) {
        result.innerHTML = '<div class="success">✅ SUCCESS!<br><div class="url" onclick="navigator.clipboard.writeText(\''+data.url+'\')">'+data.url+'</div><button onclick="navigator.clipboard.writeText(\''+data.url+'\')">📋 Copy Link</button></div>';
    } else {
        result.innerHTML = '<div style="color:red;padding:20px;">❌ Error: ' + data.error + '</div>';
    }
